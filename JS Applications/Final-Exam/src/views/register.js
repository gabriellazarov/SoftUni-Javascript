...
const registerTemplate = (formChecker) => html`
<section id="register-page" class="content auth">
    <h1>Register</h1>

    <form id="register" action="#" method="" @submit = ${formChecker}>
        <fieldset>
...
export async function registerPage(ctx) {
    ctx.render(registerTemplate(registerForm));
    ...
    async function registerForm(event) {
        ...
        if (email == '' || password == '' || repeatPass == '') return window.alert('All fields must be filled!');

        if (password !== repeatPass ) return window.alert('Passwords must match!');
